...
class TransactionController extends Controller
{
    public function index(Request $Request)
    {
        if ($Request->ajax()) {
            ...
            $Datas = DB::table('tipster_transaction')
                ->select([
                    'tipster_transaction.*', 'tipster_user.username',
                ...
                ->where('tipster_match_bet.id', (int)$Request['idMatchBet']);
            ...
            return $Datatables
                ...
                ->addColumn('action', function ($Data) {
                    ...
                    //* EDIT
                    $Html .= '<li class="nav-item"><a class="nav-link" href="' . url('admin/tipster/transaction/edit/' . $Data->id) . '"><i class="flaticon2-edit nav-icon"></i><span class="nav-text">Edit</span></a></li>';

                    //* DELETE
                    $Html .= '<li class="nav-item"><a class="nav-link btn-delete" href="' . url('admin/tipster/transaction/delete/' . $Data->id) . '"><i class="flaticon2-delete nav-icon"></i><span class="nav-text">Delete</span></a></li>';
                    $Html .= '</ul></div></div>';
        ...
        return view('pages.tipster.transaction.index');
    }
    ...
    public function DataMatchBet(Request $Request)
    {
        ...
        return $Datatables
            ...
            ->addColumn('action', function ($Data) {
                ...
                //* EDIT
                $Html .= '<li class="nav-item"><a class="nav-link" onclick="openTableTransaction(' . $Data->id . ')"><i class="flaticon2-open-box nav-icon"></i><span class="nav-text">Transaction</span></a></li>';
    ...
    public function create()
    {
        ...
        return view('pages.tipster.transaction.create', compact('DataUser'));
    }
    ...
    public function store(Request $Request)
    {
        ...
        $Rules = [
            'user_id' => "required",
            'tipster_match_bet_id' => 'required',
            'type' => ['required', 'type' => 'in:home,away'],
            'bet_type' => ['required', 'type' => 'in:big,normal'],
            'win_prize' => ['required', 'numeric', 'between:0,999999999999.999']
        ];
        ...
        $Attributes = [
            'user_id' => 'User Id',
            'tipster_match_bet_id' => 'Match Bet',
            'type' => 'Type',
            'bet_type' => 'Bet Type',
            'win_prize' => 'Win Prize',
        ];
        ...
        try {
            ...
            $Transaction = new TipsterTransaction();

            $Transaction->fill([
                'user_id' => $Data['user_id'],
                'tipster_match_bet_id' => $Data['tipster_match_bet_id'],
                'type' => $Data['type'],
                'bet_type' => $Data['bet_type'],
                'place_bet_time' => date('Y-m-d H:i:s', strtotime(Carbon::now()->timezone("GMT"))),
        ...
        if ($IsError == true) {
            ...
            return response()->json([
                ...
                'success' => true,
                'message' => $Message,
                'redirect' => url('admin/tipster/transaction')
            ], 200)->withHeaders([
                'Content-Type' => 'application/json'
            ]);
        }
    ...
    public function edit($Id)
    {
        ...
        $Data = TipsterTransaction::find($Id);

        if (empty($Data))
            return redirect('admin/tipster/transaction')->with(['error' => 'Data not exists']);

        $DataUser =  TipsterUser::orderBy('username')->get();

        $DataMatchBetTeam =  new MatchBetTeam();

        $DataMatchBetTeam = $DataMatchBetTeam->transactionEdit($Data);


        return view('pages.tipster.transaction.edit', compact('Data', 'DataUser', 'DataMatchBetTeam'));
    }
    ...
    public function update($Id, Request $Request)
    {
        ...
        try {
            ...
            $Transaction = TipsterTransaction::find($Id);
    ...
    public function delete($Id)
    {
        try {
            ...
            $delete = TipsterTransaction::where('id', $Id)->delete();

            DB::commit();

            return redirect('admin/tipster/transaction')->with(['success' => 'Transaction has been deleted successfully']);
        } catch (Exception $e) {
            DB::rollBack();
